...
def verifyCaptchaRequest(arguments):
    symbols_file = open(arguments.symbols, 'r')
    captcha_symbols = symbols_file.readline().strip()
    possibleCharacters = len(captcha_symbols) 
    max_unique_captchas = 0
    for i in range(arguments.min_length, arguments.max_length + 1):
        max_unique_captchas = max_unique_captchas + (possibleCharacters ** i)

    if arguments.count > max_unique_captchas:
        print("Error: You are requesting " + str(count) + " images to be generated but only " + str(max_unique_captchas) + " images can have unique characters")
        exit(1)
    elif arguments.count < 11:
...
def parseArguments(parser):
    ...
    argumentsAndDescriptions = {
        ...
        '--min-length': ('Minimum character length of the generated captcha images', int),
        '--max-length': ('Maximum character length of the captcha images', int),
        ...
        '--font': ('Font for the captchas', str)
    }
    ...
    verifyCaptchaRequest(arguments)
...
def main():
    ...
    font_path = './font/' + arguments.font + '.ttf'
    min_font_size = 70
    max_font_size = 100

    # Create a captcha generator for the specified width and height of our required captchas
    captcha_generator = captcha.image.ImageCaptcha(fonts=[font_path], width=arguments.width, height=arguments.height, font_sizes=(min_font_size, max_font_size))

    # Read the symbol set to generate our required captchas
    symbols_file = open(arguments.symbols, 'r')
    captcha_symbols = symbols_file.readline().strip()
    symbols_file.close()
    print("Generating captchas with the symbol set: {" + captcha_symbols + "}")
    ...
    with open(os.path.join(trainingoutput_dir, 'output.txt'), 'w') as training_output_file, (
        ...
        for i in tqdm(range(arguments.count), desc="Generating Captchas"):
            captcha_length = random.randint(arguments.min_length, arguments.max_length)
            ...
            while image_path.split("/images/")[1] in used_captcha_names:
                captcha_length = random.randint(arguments.min_length, arguments.max_length)
                random_str = ''.join([random.choice(captcha_symbols) for j in range(captcha_length)])
                image_path = os.path.join(training_dir if i < trainingdatasplit else validation_dir,
                                      random_str + '.png')
            
            used_captcha_names.add(image_path.split("/images/")[1])
